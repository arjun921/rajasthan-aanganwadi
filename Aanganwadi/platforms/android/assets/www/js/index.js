...
apisuccess=function(data,st,xhr){console.log(data.url);$('#content').show();$('#content').html('');setTitle(contentID);loadFileByType(data);};apierror=function(returnval){if(returnval.status==404){contentNotFound()}
else if(returnval.status==403){contentNoLogin()}
else{contentUnkError()}};hitApi('/content',sendData,apisuccess,apierror);}
function createNav(id){sendData={'catid':id}
apisuccess=function(data,st,xhr){setTitle(data.title)
listing=data.contains;Cookies.set('CurrPage',data.id);searchInput.oninput=searchCategories;var updateBookCount=function(numCategories){bookCountBadge.innerText=numCategories+' items';};updateBookCount(listing.length);totalCategories=listing.length;if(totalCategories>paginateSplit){$('#pagination').show();start=0;end=paginateSplit;}
else{start=0;end=totalCategories;}
...
function getIcon(fileType){if(fileType=="MP3"){return["audiotrack",]}
else if(fileType=="PDF"){return"picture_as_pdf"}
else if(fileType=="MP4"){return"video_library"}
else if(fileType=="UP"){console.log("history");return"history"}
else{return""}}
